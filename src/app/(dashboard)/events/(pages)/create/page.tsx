...
const CreateEvent = () => {
  ...
  const form = useForm<z.infer<typeof EVENT_SCHEMA>>({
    resolver: zodResolver(EVENT_SCHEMA),
    mode: 'onChange',
  });
  const {
  ...
  const { mutate, isLoading } = useMutation(EventsApi.create, {
    ...
    onError(err) {
      ...
      if (error.response?.data && error.response?.data.errors.length > 0) {
        ...
        errors.map((e) => {
          form.setError(
            e.path as unknown as keyof z.infer<typeof EVENT_SCHEMA>,
            {
              message: e.msg,
            },
          );
  ...
  function onSubmit(values: z.infer<typeof EVENT_SCHEMA>) {
    mutate(values);
  }
